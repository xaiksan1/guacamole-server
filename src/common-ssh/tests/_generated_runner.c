...
/* Automatically-generated prototypes for the sftp suite */
...
int main() {
    ...
    /* Create and register all tests for the sftp suite */
    CU_pSuite sftp = CU_add_suite("sftp", NULL, NULL);
    if (sftp == NULL
        || CU_add_test(sftp, "normalize_absolute_windows", test_sftp__normalize_absolute_windows) == NULL
        || CU_add_test(sftp, "normalize_absolute_unix", test_sftp__normalize_absolute_unix) == NULL
        || CU_add_test(sftp, "normalize_absolute_mixed", test_sftp__normalize_absolute_mixed) == NULL
        || CU_add_test(sftp, "normalize_relative_windows", test_sftp__normalize_relative_windows) == NULL
        || CU_add_test(sftp, "normalize_relative_unix", test_sftp__normalize_relative_unix) == NULL
        || CU_add_test(sftp, "normalize_relative_mixed", test_sftp__normalize_relative_mixed) == NULL
        || CU_add_test(sftp, "normalize_long", test_sftp__normalize_long) == NULL
        || CU_add_test(sftp, "normalize_deep", test_sftp__normalize_deep) == NULL
    ) goto cleanup;
